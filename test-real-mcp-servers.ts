 ...
 * Tests the pipeline with actual MCP servers from GitHub
 */
...
const REAL_MCP_SERVERS = [
  ...
  },
  {
    ...
    description: 'Interact with Notion API for page management'
  },
  {
    name: 'Obsidian MCP Server',
    url: 'https://github.com/calclavia/mcp-obsidian',
    description: 'Read and search through Obsidian vault or Markdown notes'
  }
];

async function testSingleRealServer(serverInfo: { name: string; url: string; description: string }) {
  ...
  try {
    ...
    if (!githubData) {
      log('⚠️ Could not extract GitHub data (might need GitHub token)', colors.yellow);
    ...
    // Step 3: Enrich with pipeline
    log('🌐 Step 3: Enriching with Serper + Crawl4AI + Llama...', colors.cyan);
    ...
    if (enrichmentResult.success) {
      ...
      // Calculate cost
      const cost = calculateEnrichmentCost(enrichmentResult);
      log(`   💰 Estimated cost: $${cost.toFixed(4)}`, colors.yellow);
...
function calculateEnrichmentCost(result: any): number {
  const serperCost = (result.searchesMade / 1000) * 5; // $5 per 1K searches
  const crawl4aiCost = 0; // Free with fallback
  const llamaCost = (result.tokensUsed / 1000000) * 0.27; // $0.27 per 1M tokens
  
  return serperCost + crawl4aiCost + llamaCost;
}

async function testBatchRealServers(serverCount: number = 3) {
  log('🚀 Real MCP Server Pipeline Test', colors.bright);
  log('='.repeat(60), colors.cyan);
  ...
  log(`📋 Testing ${serverCount} real MCP servers...`, colors.blue);
  ...
  // Summary
  log('\n📊 Batch Test Results Summary', colors.blue);
  log('='.repeat(60), colors.cyan);
  ...
  if (successfulResults.length > 0) {
    ...
    successfulResults.forEach((result, index) => {
      ...
      log(`      Cost: $${result.cost.toFixed(4)}, Time: ${(result.processingTime / 1000).toFixed(1)}s`, colors.cyan);
    });
  }
  ...
  if (failedResults.length > 0) {
    log('\n❌ Failed Enrichments:', colors.red);
    failedResults.forEach((result, index) => {
      log(`   ${index + 1}. ${result.serverName}`, colors.red);
      if (result.errors) {
        result.errors.slice(0, 1).forEach(error => {
          log(`      Error: ${error}`, colors.red);
        });
      }
    });
  ...
  if (successful > 0) {
    ...
    log(`💰 Cost efficiency: $${(totalCost / successful).toFixed(4)} per server`, colors.yellow);
    log(`⚡ Speed: ${(totalTime / successful / 1000).toFixed(1)}s per server`, colors.cyan);
  }

  log('\n📚 Next Steps:', colors.blue);
  log('1. Review enriched data quality', colors.cyan);
  log('2. Set up database integration', colors.cyan);
  log('3. Scale to larger batches', colors.cyan);
...
if (require.main === module) {
  ...
  testBatchRealServers(serverCount).catch(error => {
    log(`\n❌ Test suite failed: ${error}`, colors.red);
    process.exit(1);
  });
}

export { testSingleRealServer, testBatchRealServers, REAL_MCP_SERVERS };
